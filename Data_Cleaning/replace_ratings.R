...
replace_base_ratings <- function(x){
  ...
  x <- revalue(x, 
               c("GLQ" = 5, # exellent 
                 ...
                 "Rec" = 2, # fair
                 "LwQ" = 1, # low quality
                 "Unf" = NA # if unfinished we dont know the quality, let impute that later
               )
  )
  return(x)
}
...
replace_ratings <- function(X_com){
  ...
  name_standard_rating <- c("PoolQC", "GarageCond", "GarageQual", "FireplaceQu","KitchenQual","HeatingQC",
  ...
  X_com$BsmtExposure <- revalue(X_com$BsmtExposure, 
                                c("Gd" = 3,
                                  "Av" = 2,
                                  "Mn" = 1,
                                  "No" = 0
